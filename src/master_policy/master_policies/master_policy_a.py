...
class MasterPolicyACreator(MasterPolicyCreator):
    def factory_method(self, events, challenger):
        return MasterPolicyAProduct(events, challenger)


class MasterPolicyAProduct(MasterPolicyProduct):
    def __init__(self, events, challenger):
        super().__init__(events, challenger)
        self.master_policy_name = 'GENERIC_POLICY-A'
        logger.info(
